    ...
    public class EventCompletWithTransactionDto : EventCompletDto
    {
        ...
        public EventCompletWithTransactionDto ModelToDto(EventComplet eventData)
        {
            return new EventCompletWithTransactionDto()
            {
                ...
                EndDate = eventData.EndDate.ToLocalTime(),
                ...
                Name = eventData.Name,
                StartDate = eventData.StartDate.ToLocalTime(),
                ...
                Variants = eventData?.Variants.Select(v => new VariantWithLotDto()
                {
                    ...
                    Status = v.Status,
                    Lots = eventData.Lots.Where(i => i.IdVariant == v.Id && i.TotalTickets > 0).Select(l => new LotWithTicketDto()
                    {
                        EndDateSales = l.EndDateSales.ToLocalTime(),
                        Id = l.Id,
                        Identificate = l.Identificate,
                        IdVariant = l.IdVariant,
                        ReqDocs = l.ReqDocs,
                        StartDateSales = l.StartDateSales.ToLocalTime(),
                        Status = l.Status,
